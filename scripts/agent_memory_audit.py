...
AUDIT_DB = Path(
    os.path.expandvars(os.environ.get("AGENT_MEMORY_AUDIT_DB", str(CONFIG_ROOT / "audit_decisions.sqlite")))
).expanduser().resolve()
REPO_ROOT = Path(__file__).resolve().parents[1]
VAULT_ROOT = Path(
    os.path.expandvars(os.environ.get("AGENT_MEMORY_ROOT", str(REPO_ROOT / "templates" / "vault")))
).expanduser().resolve()
