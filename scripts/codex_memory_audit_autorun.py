...
AUDIT_SCRIPT = SCRIPT_ROOT / "codex_memory_audit.py"
PYTHON = os.environ.get("CODEX_MEMORY_PYTHON", sys.executable)
RUN_LOG = Path(
    os.path.expandvars(os.environ.get("CODEX_MEMORY_AUDIT_RUN_LOG", str(CONFIG_ROOT / "logs" / "audit_runs.jsonl")))
).expanduser().resolve()
LATEST_REPORT = Path(
    os.path.expandvars(os.environ.get("CODEX_MEMORY_AUDIT_REPORT", str(CONFIG_ROOT / "reports" / "latest-audit.json")))
).expanduser().resolve()
...
def run_audit(args: argparse.Namespace, last_run: dt.datetime | None) -> dict[str, Any]:
    ...
    if args.notify and payload["ok"] and payload["findings_count"]:
        ...
        notify("Codex 记忆体检", "本次 audit 已完成，未发现新的待看项。")
    elif args.notify and not payload["ok"]:
        notify("Codex 记忆体检失败", "audit 自动运行失败，请查看 audit-launchd.err.log。")
    return payload
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--stale-days", type=int, default=120, help="Forwarded stale threshold for codex_memory_audit.py.")
    parser.add_argument("--open-loop-threshold", type=int, default=4, help="Forwarded open-loop threshold for codex_memory_audit.py.")
