...
VAULT_ROOT = Path(os.path.expandvars(os.environ.get("AGENT_MEMORY_ROOT", str(REPO_ROOT / "templates" / "vault")))).expanduser().resolve()
CONFIG_ROOT = Path(os.path.expandvars(os.environ.get("AGENT_MEMORY_CONFIG_ROOT", "$HOME/.config/codex-memory"))).expanduser().resolve()
STATE_DB = Path(os.path.expandvars(os.environ.get("AGENT_MEMORY_STATE_DB", str(CONFIG_ROOT / "state.sqlite")))).expanduser().resolve()
...
def run(command: list[str], timeout: int = 300) -> dict[str, Any]:
    try:
        completed = subprocess.run(command, text=True, encoding="utf-8", errors="replace", capture_output=True, timeout=timeout, check=False)
